...
export function draw_center_line(context: CanvasRenderingContext2D) {
  //draw line with 30 dash
  let buffer = 12;
...
export function draw_scores(
  ...
  if (player1.score >= 10) {
    ...
    context.fillText(
      "player one WON!",
      module_const.canvas_width / 2,
      module_const.canvas_height / 2
    );
  }
...
export function draw_progress_bar(
  elem: HTMLElement,
  player: Paddle,
  style: string
) {
